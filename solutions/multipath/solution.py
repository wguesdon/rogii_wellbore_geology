...
# NINE-SEED AVERAGE. The GRU column is the mean of nine draws of the shipped recipe
# cmp_psr4_gr9_w2_e160 rather than three. No inference code changes: load_fold_models
# recurses from the dataset root and predict_well_drift averages whatever it is handed,
# so this is a dataset change and 45 checkpoints are loaded instead of 15.
# Ramped pooled 7.203837 -> 7.159317, paired well-cluster bootstrap -0.04384,
# CI95 [-0.10971, +0.02546], P(worse) 0.105. BELOW the 0.05 ft floor with a CI spanning
# zero; shipped for the option, not because it clears a bar.
# The ramp, SHIPPED_LAMBDA, the 0.5 mix, the post-processing cell and the GRU-free
# Ridge are all UNCHANGED and each was re-measured rather than assumed. Re-freezing the
# post-processing cell on this path is destructive at +0.0689 ft, 5 of 5 folds worse.
RIDGE_COEF = np.array(
    [
        0.5484621704458936,
        0.09262411471290159,
...
# The SECOND Ridge: the same six bases WITHOUT the GRU arm. It is not an alternative
# submission and its path is never written out. It exists only as a second, ROUGHER substrate
# for the trust-datum matcher in section 7b, which is where the whole gain lives. Derived the
# same way the seven-base one was, by a positive Ridge with alpha 1.0 fit on all 3,783,989 OOF
# rows, and checked against the banked `predictions/pred_grufree_2026_08_04.npy` at 0.000e+00 ft
# by `scripts/emit_multipath_constants_2026_08_04.py`.
#
# NOTE the coefficient on `cnn_1d_v1_avg3`. It is exactly 0.0 in the seven-base fit above and
# 0.18359 here, nonzero in the full fit and in all five cross-fit folds (0.1911, 0.1842, 0.1812,
# 0.2120, 0.1507). The CNN forward pass that was dead weight for the parent kernel is load-
# bearing for this one: deleting the column moves the GRU-free path by up to 7.554 ft.
RIDGE_MODELS_GRUFREE = [
    "lgbmede2ratecoupledfeature_2026_08_01",
    "realmlp_v1_s42",
    "cnn_1d_v1_avg3",
    "untried19_lam0.01_sg5",
...
RIDGE_COEF_GRUFREE = np.array(
    [
        0.7992949850558195,
        0.14932786200854867,
        0.18359341113002603,
        0.019848190792260783,
        0.0,
        0.022575973578061313,
    ],
    dtype=np.float64,
)


RIDGE_INTERCEPT_GRUFREE = -0.1767614241408766


def compose_path(stack_residual: np.ndarray, tag: str, report_csv: str) -> np.ndarray:
    """Blend one Ridge stack with the PF, savgol it, and U-project it.

    This is the deployed post-processing cell, unchanged, lifted into a function so the shipped
    seven-base path and the GRU-free six-base path go through the IDENTICAL block rather than
    through two copies that can drift apart.

    Args:
        stack_residual: Residual-space Ridge output, one value per test row.
        tag: Short name used to label the log lines.
        report_csv: Filename for this path's per-well projection report.

    Returns:
        The post-processed absolute TVT path, float32.
    """
    final_residual = W_STACK * stack_residual + W_PF * pf_resid
    path = (last_known_test + final_residual).astype(np.float32)
    print(f"[{tag}] Stack-only TVT mean {(last_known_test + stack_residual).mean():.2f}")
    print(f"[{tag}] Blend  TVT mean {path.mean():.2f}  std {path.std():.2f}")
    ...
    if APPLY_SG_SMOOTHING:
        ...
        path = smoothed.astype(np.float32)
        print(f"[{tag}] SG-smoothed final TVT mean {path.mean():.2f} std {path.std():.2f}")

    if APPLY_U_PROJECTION:
        path, projection_report = project_prediction_by_well(
            prediction=path,
            frame=test_df,
            test_dir=TEST_DIR,
        ...
        projection_report.to_csv(report_csv, index=False)
        if len(projection_report):
            ...
            mean_adj = float("nan")
        print(
            f"[{tag}] U projection projected {ok_count}/{len(projection_report)} wells; "
            f"mean_abs_adjustment={mean_adj:.3f}; "
            f"final mean {path.mean():.2f} std {path.std():.2f}"
        )
    return path
...
missing_bases_gf = [name for name in RIDGE_MODELS_GRUFREE if name not in model_residuals]
if missing_bases_gf:
    raise KeyError(f"Missing residual bases for GRU-free ridge stack: {missing_bases_gf}")
stack_matrix_gf = np.column_stack([model_residuals[name] for name in RIDGE_MODELS_GRUFREE])
stack_residual_gf = stack_matrix_gf @ RIDGE_COEF_GRUFREE + RIDGE_INTERCEPT_GRUFREE
print(f"[ridge-gf] 6-base intercept={RIDGE_INTERCEPT_GRUFREE:+.4f} "
      f"coef={RIDGE_COEF_GRUFREE.tolist()}")
print(f"[ridge-gf] residual mean {stack_residual_gf.mean():+.4f} "
      f"std {stack_residual_gf.std():.4f}")

prediction_tvt = compose_path(stack_residual, "shipped", "projection_report.csv")
prediction_grufree = compose_path(stack_residual_gf, "grufree",
                                  "projection_report_grufree.csv")
...
# %% [markdown]
# ## 7b. Trust-gated typewell datum correction, from TWO paths
#
# ESTIMATE-SPACE DATUM AVERAGING. Every smoothing stage in this pipeline makes the shipped path a
# worse SUBSTRATE for the matcher. Seed averaging took the per-well datum correlation from 0.27770
# to 0.24895 and the retuned cell smooths harder still, so the trajectory that scores best is not
# the trajectory that LOCALISES best. The fix costs no new information: read a second shift off the
# rougher GRU-free sub-path this kernel already computes above, and average the two shifts in
# ESTIMATE space rather than averaging the paths. Measured, and independently re-derived at both
# anchors 0.000e+00 ft in reports/verify_multipath_datum_2026_08_04.json:
#
#   rho of the shipped-path shift   0.2490
#   rho of the GRU-free-path shift  0.2990   <- the rougher path localises BETTER
#   rho of the averaged shift       0.2956
#   ramped CV 7.267606 -> 7.203837, -0.06377 ft, CI95 [-0.1134, -0.0160], p_worse 0.0053
#
# Rule 3 control, shuffling the second path's shifts across wells, gives +0.05950 and is
# destructive as required, so the gain is carried by which well each second shift belongs to and
# not by its marginal distribution. The averaged shift is applied to the SHIPPED path only; the
# GRU-free path is never written out.
#
...
# %%
#: Equal weights, FROZEN, not selected. The mixing weight was swept and the optimum sits near
#: 0.6 to 0.75 at 7.1970 against 7.2038 at 0.5, a 0.0068 ft difference far below what this CV
#: resolves and one the curve is flat over from 0.4 to 1.0. Buying it would be selection over
#: all 773 wells, so it is not bought.
MULTIPATH_MIX = 0.5

_trust_shifts = []
_pred64 = np.asarray(prediction_tvt, dtype=np.float64).copy()
_gf64 = np.asarray(prediction_grufree, dtype=np.float64)
_n_gf_fallback = 0
for _wid, _gidx in test_df.groupby("well_id", sort=False).groups.items():
    ...
    if int(_good.sum()) < MIN_WINDOW_ROWS:
        ...
        continue
    _tw_tvt = _tw["TVT"].to_numpy(dtype=np.float64)
    _tw_gr = _tw["GR"].to_numpy(dtype=np.float64)
    # Call 1: the shipped path, exactly as the parent kernel called it.
    _s1 = shipped_datum_shift(_md[_good], _gr[_good], _p[_good], _tw_tvt, _tw_gr)
    # Call 2: the SAME matcher on the rougher GRU-free path, same rows, same typewell.
    if np.all(np.isfinite(_q[_good])):
        _s2 = shipped_datum_shift(_md[_good], _gr[_good], _q[_good], _tw_tvt, _tw_gr)
    else:
        _n_gf_fallback += 1
        _s2 = _s1
    _s = (1.0 - MULTIPATH_MIX) * _s1 + MULTIPATH_MIX * _s2
    ...
    _trust_shifts.append({"well_id": _wid, "shift_ft": float(_s), "shift_shipped_ft": float(_s1), "shift_grufree_ft": float(_s2), "applied_min": float(_s * _scale.min()), "applied_max": float(_s * _scale.max()), "reason": "ok"})
...
if _n_gf_fallback:
    print(f"WARNING: {_n_gf_fallback} wells had a non-finite GRU-free path; "
          f"those fell back to the shipped-path shift alone.")
...
if len(_nz):
    print(f"Multipath: mean |shipped shift| {_nz['shift_shipped_ft'].abs().mean():.3f} ft; "
          f"mean |GRU-free shift| {_nz['shift_grufree_ft'].abs().mean():.3f} ft; "
          f"corr {_nz['shift_shipped_ft'].corr(_nz['shift_grufree_ft']):.4f}")
